...
pub trait Downstream: Send + Sync {
    ...
    fn header_connected(&mut self, header: &Header, height: u32);

    /// called by the node if a block is removed from trunk (orphaned from longest chain)
    fn block_disconnected(&mut self, header: &Header);
}
...
impl Downstream for DownStreamDummy {
    ...
    fn header_connected(&mut self, _header: &Header, _height: u32) {}

    fn block_disconnected(&mut self, _header: &Header) {}
}
